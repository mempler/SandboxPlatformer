...
static std::string generateCodeForMeta( MetadataInfo metaData )
{
    ...
    // Generate header
    code << "// !!! This code is generated and might be changed DO NOT EDIT !!!"
         << std::endl
         << std::endl;

    code << "#include \"Kokoro/Memory.hh\"" << std::endl << std::endl;
    code << "struct " << metaData.m_sStructName << std::endl
         << "{" << std::endl;

    parser_code << "    void unpack(Kokoro::Memory::Buffer& buff)" << std::endl
                << "    {" << std::endl;
    ...
    for ( size_t i = 0; i < metaData.m_vSchema.size(); i++ )
    {
        ...
        for ( auto s : schema )
        {
            ...
            else if ( s.second == "[..]byte" )
            {
                ...
                // TODO:
                parser_code << "size_t sSize_" << si
                            << " = buff.Pop<size_t>(4);" << std::endl;
                parser_code << "        ";
                parser_code << "std::string sBufferData_" << si
                            << " = buff.Pop<std::string>(sSize_" << si << ");"
                            << std::endl;
                parser_code << "        ";
                parser_code << "m_" << c
                            << " = "
                               "Kokoro::Memory::Buffer(std::vector<uint8_t>("
                               "sBufferData_"
                            << si << ".begin(), "
...
std::pair<MetadataInfo, IParser *> getMetaParserByExt(
    const std::string_view &svExt )
{
    for ( auto meta : g_vMetadata )
    {
        // Try to find the correct metadata
        //-----------------------------------
        if ( std::find( meta.m_vsExtensions.begin(), meta.m_vsExtensions.end(),
                        svExt )
             != meta.m_vsExtensions.end() )
        {
            // We found it!
...
#define IMPL_SIMPLE_DATA_TYPE( S, T )                                          \
    {                                                                          \
        ...
        if ( v == nullptr )                                                    \
        {                                                                      \
            LOG_WARN << "Unimplemented key in parser: " << s.first             \
                     << std::endl;                                             \
        }                                                                      \
        else                                                                   \
        {                                                                      \
...
Kokoro::Memory::Buffer constructBinaryFromMeta( IParser *parser,
                                                MetadataInfo meta )
{
    ...
    for ( auto s : meta.m_vSchema [ meta.m_uVersion ] )
    {
        ...
        else if ( s.second == "[..]byte" )
        {
            std::vector<uint8_t> *v =
                parser->getValue<std::vector<uint8_t>>( s.first );
...
{
    std::cout << "ASCP (Asset Compiler) v1.0.0" << std::endl
              << "Copyright by Robin A. P. 2021" << std::endl
              << std::endl;
    ...
    {
        std::vector<uint8_t> metaFileRawBinary =
            Kokoro::FileSystem::ReadBinaryFile( metaFilePath );
        std::string metaFileRawJson =
            std::string( metaFileRawBinary.begin(), metaFileRawBinary.end() );
        ...
        for ( size_t i = 0; i < array_count( metaDataFile.elements ); i++ )
        {
            ...
            if ( strcmp( activeElement.name, "" ) == 0 ) continue;
            ...
            {
                for ( size_t i = 0; i < array_count( activeElement.elements );
                      i++ )
                {
                    auto activeFlagElement = activeElement.elements [ i ];

                    if ( activeFlagElement.string == nullptr ) continue;
            ...
            else if ( strcmp( activeElement.name, "extensions" ) == 0 )
            {
                      ...
                      i++ )
                {
                    ...
                    if ( activeExtElement.string == nullptr ) continue;
            ...
            else if ( strcmp( activeElement.name, "schema" ) == 0 )
            {
                      ...
                      i++ )
                {
                    ...
                    for ( size_t i = 0;
                          i < array_count( activeSchemaElement_.elements );
                          i++ )
                    {
                        auto activeSchemaElement =
                            activeSchemaElement_.elements [ i ];

                        info.m_vSchema.back().insert(
                            std::make_pair( activeSchemaElement.name,
                                            activeSchemaElement.string ) );
                    }
                }
            }
            else
            {
                LOG_WARN << "Key not implemented: " << activeElement.name
                         << std::endl;
            }
        }
    ...
    if ( generateCode )
    {
        ...
        for ( auto metaInfo : g_vMetadata )
        {
            ...
            std::string outputPath = Kokoro::FileSystem::JoinPath(
                output_path, metaInfo.m_sStructName + ".hh" );
    ...
    std::string_view file_name_no_ext =
        file_name.substr( 0, file_name.rfind( '.' ) );
    std::string file_extension =
        input_file.substr( input_file.rfind( '.' ) + 1 );

    auto metaParser = getMetaParserByExt( file_extension );
    if ( metaParser.second == nullptr )
    {
        std::cout << "FATAL> Missing metadata for ext:" << file_extension
                  << std::endl;
        exit( 1 );
    }

    Kokoro::Memory::Buffer buffer(
        Kokoro::FileSystem::ReadBinaryFile( input_file ) );

    metaParser.second->performParse( input_file, buffer );

    Kokoro::Memory::Buffer binaryBuffer =
        constructBinaryFromMeta( metaParser.second, metaParser.first );

    std::string outputPath = Kokoro::FileSystem::JoinPath(
        output_path,
        std::string( file_name_no_ext.begin(), file_name_no_ext.end() )
            + metaParser.first.m_sExt );
