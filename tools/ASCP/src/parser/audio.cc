...
void AudioParser::performParse(const std::string_view &svFileName, Kokoro::Memory::Buffer &bInputBuffer) {
    ...
    pcmFrames.resize(wav.totalPCMFrameCount * wav.channels * (wav.bitsPerSample / 8));

    size_t numberOfSamplesActuallyDecoded = drwav_read_pcm_frames(&wav, wav.totalPCMFrameCount, (void *)pcmFrames.data());
    ...
    if (wav.channels > 1 && wav.channels <= 2) {
        ...
        MOVE_TO_HEAP(std::vector<uint8_t>, channelRightData, std::vector<uint8_t>()); // Empty, just mono
