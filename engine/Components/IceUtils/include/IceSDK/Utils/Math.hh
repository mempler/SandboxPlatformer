...
namespace IceSDK
{
    namespace Math
    {
        inline glm::vec2 ApplyScale( glm::vec2 v2Target, glm::vec2 v2Original )
        {
            ...
            float scaleFactor = targetAspect > originalAspect ? v2Target.y / v2Original.y
                                                              : v2Target.x / v2Original.x;

            return glm::vec2( scaleFactor );
        }

        inline glm::mat4 CalcTransform( const glm::vec3 &v3Position,
                                        const glm::vec2 &v2Size, float fRotation = 0.f,
                                        const glm::vec3 &v3RotationAxis = { 1, 0, 0 } )
        {
            return glm::translate( glm::mat4( 1.f ), glm::ceil( v3Position ) )
