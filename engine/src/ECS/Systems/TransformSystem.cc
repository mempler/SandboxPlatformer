...
void TransformSystem::Tick(float pDelta) {
    ...
    for (auto rawTransformEntity : transformGroup) {
        ...
        auto &transform = spriteEntity.GetComponent<Components::TransformComponent>();

        if (!transform._hasChanged)
            continue;
        ...
        matrix = glm::rotate(matrix, glm::radians(transform.m_fRotation), glm::vec3(0.f, 0.f, 1.f));
