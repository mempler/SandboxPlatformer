...
#define Xor_( str )                                                            \
    ::jm::xor_string(                                                          \
        ...
        std::make_index_sequence<                                              \
            ::jm::detail::_buffer_size<sizeof( str )>()> {} )
...
namespace jm
{
    ...
    namespace detail
    {
        ...
        XORSTR_FORCEINLINE constexpr std::uint32_t key4() noexcept
        {
            ...
            for ( char c : __TIME__ )
                value =
                    static_cast<std::uint32_t>( ( value ^ c ) * 16777619ull );
            return value;
        }
        ...
        // loads up to 8 characters of string into uint64 and xors it with the
        // key
            ...
            std::uint64_t key, std::size_t idx, const CharT *str ) noexcept
        {
            ...
            for ( std::size_t i = 0; i < idx_offset && i + idx * idx_offset < N;
                  ++i )
                value ^= ( std::uint64_t { static_cast<cast_type>(
                               str [ i + idx * idx_offset ] ) }
                           << ( ( i % idx_offset ) * 8 * value_size ) );

            return value;
        }

        // forces compiler to use registers instead of stuffing constants in
        // rdata
        XORSTR_FORCEINLINE std::uint64_t load_from_reg(
                     ...
                     std::index_sequence<Indices...>>
    {
        ...
        constexpr static inline std::uint64_t alignment =
            ( ( Size > 16 ) ? 32 : 16 );
#else
      ...
      public:
        ...
        XORSTR_FORCEINLINE xor_string(
            L l, std::integral_constant<std::size_t, Size>,
            std::index_sequence<Indices...> ) noexcept :
            _storage { JM_XORSTR_LOAD_FROM_REG(
                detail::uint64_v<detail::load_xored_str8<Size>(
        ...
        XORSTR_FORCEINLINE void crypt() noexcept
        {
            ...
            std::uint64_t *keys = (std::uint64_t *) JM_XORSTR_LOAD_FROM_REG(
                (std::uint64_t) arr );
#else
            alignas( alignment )
                std::uint64_t keys [] { JM_XORSTR_LOAD_FROM_REG( Keys )... };
#endif
    ...
    || defined( __arm__ )
            ...
            if constexpr ( sizeof( _storage ) % 32 != 0 )
                _mm_store_si128(
                    reinterpret_cast<__m128i *>( _storage + sizeof...( Keys )
                                                 - 2 ),
                    _mm_xor_si128(
        ...
        {
            // crypt() is inlined by hand because a certain compiler with a
            // certain linker is _very_ slow
#if defined( __clang__ )
            alignas( alignment )
                std::uint64_t arr [] { JM_XORSTR_LOAD_FROM_REG( Keys )... };
