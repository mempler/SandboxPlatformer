...
Audio *AudioChannel::CreateAudio( Identifier const &rIdent, uint32_t hSrcLeft,
                                  uint32_t hSrcRight, uint32_t hBufferLeft,
                                  uint32_t hBufferRight )
{
    return &m_vLinkedAudio.emplace_back( this, rIdent, hSrcLeft, hSrcRight,
                                         hBufferLeft, hBufferRight );
}
...
void AudioChannel::SetVolume( float fVolume )
{
    ...
    {
        audio.SetVolume(
            audio.GetVolume() );  // Each audio file should get the volume this
                                  // channel by default.
    }
}