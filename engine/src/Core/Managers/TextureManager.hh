  ...
  public:
                                       ...
                                       uint64_t u64Filters, glm::vec4 v4Color )
    {
        ...
        Texture2D::LoadRaw( resource, identifier, iWidth, iHeight,
                            bgfx::TextureFormat::RGBA8, u64Filters,
                            textureDataPtr );
    ...
    Texture2D *CreateTextureFromMemory(
        Identifier const &identifier, int32_t iWidth, int32_t iHeight,
        bgfx::TextureFormat::Enum eTextureFormat, uint64_t u64Filters,
        uint8_t *pData, uint32_t uDataSize )
    {
        ...
        Texture2D *resource = CreateEmpty( identifier );
        Texture2D::LoadRaw( resource, identifier, iWidth, iHeight,
                            eTextureFormat, u64Filters, textureDataPtr );

        return resource;
    }

    Texture2D *CreateTextureFromFile( Identifier const &identifier,
                                      uint64_t u64Filters )
    {
        ZoneScoped;
        // Return cached texture
