...
{
  public:
    Texture2D *CreateTextureWithColor( int32_t iWidth, int32_t iHeight,
                                       uint64_t u64Filters, glm::vec4 v4Color )
    {
        ZoneScoped;
        uint32_t data = glm::packUnorm4x8( v4Color );
        Identifier identifier =
            fmt::format( "color://{:x}", data );  // Return cached texture
        if ( Has( identifier ) ) return Load( identifier );

        // Otherwise we create it
        Kokoro::Memory::Span<uint8_t> textureDataPtr =
            Kokoro::Memory::Span( (uint8_t *) &data, 4 );

        Texture2D *resource = CreateEmpty( identifier );
        Texture2D::LoadRaw( resource, identifier, iWidth, iHeight,
                            bgfx::TextureFormat::RGBA8, u64Filters, textureDataPtr );

        return resource;
    }

    Texture2D *CreateTextureFromMemory( Identifier const &identifier, int32_t iWidth,
                                        int32_t iHeight,
                                        bgfx::TextureFormat::Enum eTextureFormat,
                                        uint64_t u64Filters, uint8_t *pData,
                                        uint32_t uDataSize )
    {
        ZoneScoped;
        ...
        // Otherwise we create it
        Kokoro::Memory::Span<uint8_t> textureDataPtr =
            Kokoro::Memory::Span( pData, uDataSize );

        Texture2D *resource = CreateEmpty( identifier );
        Texture2D::LoadRaw( resource, identifier, iWidth, iHeight, eTextureFormat,
                            u64Filters, textureDataPtr );
