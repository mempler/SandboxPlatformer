/*
    ...
    r-lyeh (fork)
    Version History:
    2.1.0 - negative numbers fix, comment parsing fix, invalid memaccess fix
   (@r-lyeh) 2.0.9 - zpl 4.0.0 support 2.0.8 - Small cleanup in README and test
   file 2.0.7 - Small fixes for tiny cpp warnings 2.0.5 - Fix for bad access on
   deallocation 2.0.4 - Small fix for cpp issues 2.0.3 - Small bugfix in name
   with underscores 2.0.1 - Catch error in name 2.0.0 - Added basic error
   handling 1.4.0 - Added Infinity and NaN constants 1.3.0 - Added multi-line
   backtick strings 1.2.0 - More JSON5 features and bugfixes 1.1.1 - Small
   mistake fixed 1.1.0 - Basic JSON5 support, comments and fixes 1.0.4 - Header
   file fixes 1.0.0 - Initial version
*/

/*
@todo change api to:
json_value * json_parse (const json_char * json, size_t length, char *error); //
error = buffer to store error, if any. void json_value_free(json_value *); The
type field of json_value is one of: json_array (see ->len, ->values[x])
...
void *arealloc(void *p, size_t sz) {
    ...
    if (p && sz) {
        size_t slen = asize(p);
        size_t *ret = (size_t *)(sz <= slen ? (size_t *)p - 1 : realloc((size_t *)p - 1, sizeof(size_t) + sz));
        *ret = sz;
        return &ret[1];
    }
    return 0;
...
void *vrealloc(void *p, size_t sz) {
    if (!p && sz) {
        size_t *ret = (size_t *)malloc(sizeof(size_t) * 2 + sz);
        ret[0] = sz;
        ret[1] = 0;
        return &ret[2];
    }
...
// default allocator
// static __thread msize_t   MSIZE   = _msize; // _msize (win),
// malloc_usable_size (linux), malloc_size (osx) static __thread realloc_t
// REALLOC = realloc;
...
#define array_append(v, t, x) (t = (v)REALLOC(t, (array_count(t) + 1) * sizeof(t[0])), t[array_count(t) - 1] = (x))
...
inline bool json5__validate_name(char *s, char *err) {
    while (*s) {
        if ((s[0] == '\\' && !json5__is_control_char(s[1])) && (s[0] == '\\' && !isxdigit(s[1]) && !isxdigit(s[2]) && !isxdigit(s[3]) && !isxdigit(s[4]))) {
            *err = *s;
            return false;
        }
        ++s;
...
char *json5__parse_array(json5_object *obj, char *base, int *err_code) {
    ...
    while (*p) {
        ...
        array_append(struct json5_object *, obj->elements, elem);
...
char *json5__parse_object(json5_object *obj, char *base, int *err_code) {
    ...
    l_parsed:
        ...
        array_append(struct json5_object *, obj->nodes, node);
...
int main() {
    ...
    printf("Category: %s, air date: %s\nQuestion: %s\n", root.nodes[0].elements[29].nodes[0].string, root.nodes[0].elements[29].nodes[1].string,
